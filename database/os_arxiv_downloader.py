...
out_dir = r"G:\oe_paper"
# 创建数据库引擎
engine = create_engine('mysql+mysqlconnector://root:1q2w3e4r5t@localhost/objective_eval')
...
def main(session):
    ...
    # Assuming abbreviations are in <strong> tags (you'll need to update this based on the actual page structure)
    abbreviations = [tag.get_text().split(' ')[0].strip() for tag in soup.find_all('h4')]

    # # Print or process the abbreviations
    # for abbr in abbreviations:
    #     print(abbr)
    key_words = abbreviations[1:]
    ...
    # 将字典保存为JSON文件
    import json
    with open("kwd_couont_oe.json", "w") as json_file:
